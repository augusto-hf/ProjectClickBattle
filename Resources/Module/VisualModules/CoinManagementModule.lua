...
local coin_info = require "Resources.Module.InfoModules.CoinInfoModule"
...
function coin_management.look_for_coin(money_value_to_look_for_coin)
	...
	while i < (how_many_coins + 1) do
		...
		if i < how_many_coins then
			current_value = math.random( (max_coin_value * minimum_coin_value_percentage) , max_coin_value)
		else
